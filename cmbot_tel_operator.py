...
import json, time, traceback, os, datetime, calendar
...
class Authenticator:
    ...
    def __init__(self):
        load_dotenv()
        self.token = os.environ.get('CMB_TOKEN')
...
class TelegramOperator:
    @staticmethod
    def waiter_greeting(update: Update, context: CallbackContext) -> str:
        print(str(update.message.from_user) + ' contacted the bot.')
        # context.chat_data
        if update.message.chat.type != 'private':
            update.message.reply_text('Привет.\nПрости, но я не разговариваю в чатах.\nПиши в личку.')
            if update.message.chat_id != curr_auth.choir_id:  # failsafe in case bot SOMEHOW gets added where it shouldn't be
                update.message.reply_text('И, кажется, я вас не знаю... Извините.')
                update.message.chat.leave()
            return ConversationHandler.END
        # if update.message.from_user.id == curr_auth.owner_id or True:
        if update.message.from_user.id in memberlist_loader():
            update.message.reply_text(text='Здравствуй. Начнём сначала.\nЧто ты хочешь сделать?',
                                      reply_markup=KEYBOARD_HOME)
            beast = update.message.from_user
            update.message.bot.send_message(chat_id=curr_auth.owner_id,
                                            text='#ALARM\nCurrently speaking with:\n' + str(beast))
        else:
            update.message.reply_text("Мне кажется, мы не знакомы :'(\nЯ не буду с тобой разговаривать")
            beast = update.message.from_user
            update.message.bot.send_message(chat_id=curr_auth.owner_id,
                                            text='#ALARM\n' + str(beast) + '\ntried to contact me')
            return ConversationHandler.END
    ...
    @staticmethod
    def waiter(update: Update, context: CallbackContext, owner_id=int) -> str:
        try:
        ...
        if input_1 == 'добавить песню':
            update.message.reply_text('Ура! Добавляем новую песню!',
                                      reply_markup=CLEAR_KEYBOARD)
            return 'ADD'
        elif input_1 == 'искать везде':
            keyboard_markup = ReplyKeyboardMarkup([['Отмена']])
            update.message.reply_text('Хорошо!', reply_markup=keyboard_markup)
            update.message.reply_text('Введи фразу (или кусок слова) по которому мне искать песню.')
            context.user_data.update({'SEARCH_KEY': ''})
            return 'SEARCH'
        elif input_1 == 'искать по названию':
            keyboard_markup = ReplyKeyboardMarkup([['Отмена']])
            update.message.reply_text('Хорошо, ищем по названию!', reply_markup=keyboard_markup)
            update.message.reply_text('Введи фразу (или кусок слова) по которому мне искать песню.')
            context.user_data.update({'SEARCH_KEY': 'name'})
            return 'SEARCH'
        elif input_1 == 'искать по теме':
            keyboard_markup = ReplyKeyboardMarkup([['Отмена']])
            update.message.reply_text('Хорошо, ищем по теме!', reply_markup=keyboard_markup)
            update.message.reply_text('Введи фразу (или кусок слова) по которому мне искать песню.')
            context.user_data.update({'SEARCH_KEY': 'theme'})
            return 'SEARCH'
        elif input_1 == 'показать':
            update.message.reply_text('Показываю ВСЕ песни в алфавитном порядке:')
            TelegramOperator.show(update, context)
            # keyboard_markup = ReplyKeyboardMarkup([['Да', 'Нет', 'Наверное']])
            update.message.reply_text('Что-то ещё?')
            return 'WAITER'
        elif input_1 == 'выбрать по номеру':
            keyboard_markup = ReplyKeyboardMarkup([['Отмена']])
            update.message.reply_text('Ты знаешь номер песни? Отлично! Скажи его:', reply_markup=keyboard_markup)
            sl = SongList(F_PATH)
            context.user_data.update({'SONGLIST': sl})
            return 'SELECT_BY_ID'
            # TelegramOperator.select_song_by_id(update, context)
        elif input_1 == 'make backup':
            if update.message.from_user.id == curr_auth.owner_id:
                TelegramOperator.make_backup()
                update.message.reply_text("BACKUP WAS INITIALIZED!")
                update.message.reply_text('Backup complete!')
            else:
                context.bot.send_message(chat_id=curr_auth.owner_id, text="BACKUP INITIALIZATION WAS ATTEMPTED!")
            return 'WAITER'
        # elif input_1 == 'load backup':
        #     TelegramOperator.load_backup()
        elif input_1 == 'закончить':
            update.message.reply_text("Ладно. Пока!", reply_markup=CLEAR_KEYBOARD)
            return ConversationHandler.END
        elif input_1 == 'debug':
            # use this for testing pieces of code
            pass
        else:
            update.message.reply_text('Я не понял, чего от меня хотят. Попробуй снова.')
    ...
    def search(update: Update, context: CallbackContext) -> str:
        ...
        if type(search_key) is not str:
            update.message.reply_text(text=FATAL_ERROR_STR, reply_markup=CLEAR_KEYBOARD)
            return ConversationHandler.END
        input_search = update.message.text
        input_search = input_search.lower()
        if input_search == "отмена":
            update.message.reply_text("Оке! Поиск отменён.")
            update.message.reply_text('Что-то ещё?', reply_markup=KEYBOARD_HOME)
            return 'WAITER'
        elif input_search != '':
            sl_search = SongList(F_PATH)
            matching_songs = sl_search.search(input_search, search_key)
            if matching_songs.sl == {}:
                update.message.reply_text("Я ничего не нашёл. :'(\nПопробуй что-то другое.")
                return None
            else:
                update.message.reply_text('Вот песни, которые я нашёл:')
                out_str = matching_songs.names_perf()
                if len(out_str) < 4000:
                    update.message.reply_text(out_str)
                    update.message.reply_text('Скажи номер произведения, который тебя интересует')
                    context.user_data.update({'SONGLIST': matching_songs})
                    return 'SELECT_BY_ID'
                else:
                    update.message.reply_text('Список результатов слишком длинный.\nПопробуй короче. :)')
                    return None
        else:
    ...
    @staticmethod
    def song_select_action(update: Update, context: CallbackContext) -> str:
        try:
            ...
            update.message.reply_text('Что-то во мне сломалось... Пойду чаю попью.')
            return ConversationHandler.END
        ...
        if input_action == "смотреть исполнения":
            ...
            if datelist:
                update.message.reply_text('Эта песня исполнялась в следующие даты:')
                out_str = ''
                for dt in datelist:
                ...
                update.message.reply_text("Исполнений не найдено")
                return 'SONG_SELECTED'
        elif input_action == "добавить исполнение":
            if update.message.from_user.id in curr_auth.admins:
                keyboard_markup = ReplyKeyboardMarkup([['Последнее воскресенье'], ['Прошлое воскресенье'], ['Отмена']])
                update.message.reply_text('Ок. Добавляем исполнение.', reply_markup=keyboard_markup)
                update.message.reply_text('Введи дату в формате ГГГГ-ММ-ДД, или выбери из списка ниже')
                return 'ADD_PERFORMANCE'
            else:
                update.message.reply_text('Тебе сюда нельзя, прости =/')
                return None
        elif input_action == 'назад':
            keyboard_markup = ReplyKeyboardMarkup([['Отмена']])
            update.message.reply_text('Хорошо, выбери другую песню из последнего списка.', reply_markup=keyboard_markup)
            return 'SELECT_BY_ID'
        elif input_action == "домой":
            update.message.reply_text('Оке. Давай сначала.', reply_markup=KEYBOARD_HOME)
            return 'WAITER'
        else:
            update.message.reply_text("Я тебя не понял. Попробуй снова.")
            return 'SONG_SELECTED'
    ...
    @staticmethod
    def select_song_by_id(update: Update, context: CallbackContext) -> str:
        try:
        ...
        if song_id in sl1.song_ids():
            update.message.reply_text('Есть такая песня, да.')
            # context.user_data.pop('SONGLIST')
            context.user_data.update({'SONG_ID': song_id})
            sl2 = SongList(F_PATH)
            song_1 = sl2.get_song(song_id)
            update.message.reply_text("Показываю песню:")
            update.message.reply_text('#' + song_id + '\n' + str(song_1))
            keyboard_markup = ReplyKeyboardMarkup([['Смотреть исполнения', 'Добавить исполнение'],
                                                   ['Назад', 'Домой']])
            update.message.reply_text('Что будем делать с песней?', reply_markup=keyboard_markup)
            return 'SONG_SELECTED'
            # TelegramOperator.song_select_action(song_id)
        elif song_id.lower() == 'отмена':
            update.message.reply_text('Ок. Начнём снова', reply_markup=KEYBOARD_HOME)
            return 'WAITER'
        else:
    ...
    @staticmethod
    def add_performance(update, context):
        keyboard_markup = ReplyKeyboardMarkup([['Смотреть исполнения', 'Добавить исполнение'],
                                               ['Назад', 'Домой']])
        song_id = context.user_data.get('SONG_ID')
        ...
        if input_date.lower() == 'отмена':
            update.message.reply_text('Хорошо. Что делать с песней?', reply_markup=keyboard_markup)
            return 'SONG_SELECTED'
    ...
    def stop(update: Update, context: CallbackContext):
        update.message.reply_text('Я показал что мог. Пиши /start, чтобы начать сначала', reply_markup=CLEAR_KEYBOARD)
        return ConversationHandler.END
...
# wd_now = int(datetime.datetime.now().strftime("%w"))
# last_s = datetime.date.today() - datetime.date.day

# TelegramOperator.waiter()
